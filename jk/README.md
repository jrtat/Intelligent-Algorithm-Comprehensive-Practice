...
定位：网站全局固定的顶部导航组件，是全站的核心导航入口，默认展示在页面最顶端。结构与功能：
左侧：品牌标识区（「AI 职业规划」Logo，点击可返回首页）
中间：导航菜单区（包含「首页」「岗位洞察」「能力测评」「专属报告」等路由入口，用于页面间跳转）
右侧：核心操作按钮区（「立即测评」主按钮，引导用户进入核心业务流程）
...
左侧：核心卖点 / 流程展示区（如「能力分析 → 岗位匹配 → 适配排名」核心流程，或「更新、定制、安全、快捷」等产品优势文案）
右侧：强引导主按钮区（如「立即测评」「下载」等高转化操作按钮，视觉焦点突出）
视觉特征：圆角卡片式设计、高对比配色（如蓝底白字）、强视觉冲击力，用于强化用户行动意愿，仅服务于首页转化。
二、切换触发时机（精准可落地）
