...
月薪范围（薪资范围）：整型列表（[最低, 最高]），日薪转化为月薪使用中国劳动法规定的月计薪天数21.75

所属行业（所属行业）：列表（不重复）

公司规模（公司规模）：20人以下 / 20-99人 / 100-299人 / 299-499人 / 500-999人 / 1000-9999人 / 10000人以上 / nan
...
实习能力（岗位详情）：1 / 2 / 3 / 4（无 / 6月以下 / 1年以下 / 1年以上）大模型打分

学历要求（岗位详情）：无任何要求 / 中专及以上 / 大专及以上 / 本科及以上 / 硕士及以上 / 博士及以上

毕业院校要求（岗位详情）：无任何要求 / 双一流 / 211 / 985

城市水平（地址）：一线 / 新一线 / 二线 / 二线以下 / 外派

（不需要）企业类型（公司类型）：Soar / Rise / Strong / Stable / Unknown
...
2. 建立岗位间的关联图谱（已完成）
	1. 垂直：内置于岗位，直接查（补充）
	2. 换岗：Top 3关联，随后分析需要掌握哪些知识（两两对比）

3. 学生就业能力画像：
	1. 录入（前端负责）√
	2. 就业能力画像（*）√
	3. 完整度、竞争力评分（*）
	4. 岗位画像和就业能力画像匹配，人岗匹配度分析（*）
		基础要求（专业，毕业院校）
		职业技能（Skill）
		职业素养（Quality）
		发展潜力（薪资，福利，晋升（涵转岗））
	5. 探索结果与个人意愿制定目标（*）：
		从探索结果筛选出符合个人意愿的项
	6. 分析本职业的社会需求与行业发展趋势
	7. 分析企业给定的职业岗位数据的关联性（感觉就是构建企业画像）
	8. 构建本职业清晰的发展路径
...
processor/
  ├── data/                          # 存放数据文件
  │   ├── cities.json                # 城市信息
  │   ├── cleaned_data.json          # 清洗后的数据（包括岗位、公司、城市、行业的初始数据）
  │   ├── companies.json             # 公司信息
  │   ├── industries.json            # 行业信息
  │   ├── jobs.json                  # 岗位信息
  │   └── README.md
  │
  ├── utils/                       # 工具文件夹
  │   ├── __pycache__/           # Python 编译后的字节码文件
  │   │   ├── ExcelMaker.cpython-37.pyc
  │   │   ├── FileProcessor.cpython-37.pyc
  │   │   └── LLMInvoker.cpython-37.pyc
  │   │
  │   ├── ExcelMaker.py          # Excel 文件生成工具
  │   ├── FileProcessor.py       # 文件处理工具
  │   ├── LLMInvoker.py          # 调用大语言模型的工具
  │   └── README.md
  │
  ├── data.xlsx                    # 原始数据文件
  ├── init.py                      # 初始化文件
  ├── json2xlsx.py                 # JSON 转 Excel 脚本
  ├── output.xlsx                  # 输出 Excel 文件
  ├── process.py                   # 主处理脚本
  ├── README.md                    # 项目说明文件
  └── reset.py                     # 重置脚本
```
...
> - init.py：将原data.xls文件提取岗位、公司、城市、类型等实体类，统一保存到cleaned_data.json。
...
modelscope download --model Qwen/Qwen2.5-3B-Instruct --local_dir ./model/Qwen2.5-3B-Instruct # 把Qwen2.5-3B下载到当前目录的/model/子文件夹里
# 1. 显存大的可以用更大的模型
# 2. 模型保存的地址可以自行更改，默认路径值：C:\Windows\System32\model\Qwen2.5-7B 
...
### payload是什么？
> payload是数据传输的载体，是发送给 API 服务器的请求参数包。它是一种数据格式，用于在计算机网络中传输数据。

### Ollama API常用的payload参数
...
payload = {
    ...
    "prompt": "写一首诗",
    "options": {
        "temperature": 0.7,     # 创造性：0（确定）~1（随机），越高越有创意
        "top_p": 0.9,           # 核采样：从累积概率 top_p 的词中采样
        "top_k": 40,            # 从 top_k 个词中采样下一个词
        "num_predict": 100,     # 最多生成多少个 token
        "repeat_penalty": 1.1,  # 重复惩罚：越高越少重复
        "seed": 42              # 随机种子：固定后结果可复现
    }
}
```

system（string）：系统提示语（可用于指示设定角色），用于指导模型生成更符合要求的结果。

template（string）：自定义提示模板，用于生成更符合要求的结果。

context（list）：上下文，用于提供模型生成更符合要求的结果。
