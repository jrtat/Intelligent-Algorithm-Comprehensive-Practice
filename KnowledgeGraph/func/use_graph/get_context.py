...
def get_merge_val_for_doc(
    ...
    if prev_text and prev_emb:
        sim_prev = cosine_similarity(value_embedding, prev_emb)
        candidates.append((sim_prev, prev_text))
    if next_text and next_emb:
        sim_next = cosine_similarity(value_embedding, next_emb)
...
def get_knowledge_merge_vals(
    job_type_id: int,
    knowledge_type: str, # "综合素质"、"职业技能"、"证书"、"工作内容"、"专业"、"工作经验" 之一
    ...
    # Step 1: 从知识节点取出文本值
    value_record = graph.query(
        f"""
            MATCH (n:`{knowledge_type}`)
            WHERE id(n) = $node_id
    ...
    for doc_id in doc_ids:
        merge_val = get_merge_val_for_doc(
            graph=graph,
            doc_id=doc_id,
            value_embedding=value_embedding
        ) # 使用辅助函数
