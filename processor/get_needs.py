...
def get_job_type_chunks(job_type_id: int, source_type: str, source_value: str) -> str:
    ...
    if source_type == "knowledge":
        ...
        records = graph.query(
            """
            MATCH (jt:职业类别)-[:属于]-(job:岗位)-[r]->(n:`{knowledge_type}`)
            WHERE id(jt) = $job_type_id
              AND type(r) = $rel_type

            MATCH (doc:Document)-[:MENTION]->(job)
            MATCH (chunk:Chunk)-[:FROM_DOCUMENT]->(doc)

            RETURN DISTINCT chunk.text AS chunk_text
            """.format(knowledge_type=source_value),
        ...
        print(f"job_type_id={job_type_id}, knowledge_type={source_value} → 找到 {len(chunks)} 个 Chunk")
        return merge_val
